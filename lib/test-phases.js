...
function InvocationEvent(runtime, input, tags, resolve) {
...
CaptureController.prototype = {
  ...
  asyncPhaseInvoked: function(phaseRuntime, input, tags, resolve) {
    this.invocations.push(new InvocationEvent(phaseRuntime, input, tags, resolve));
    this.resolveExpect();
  },
  ...
  resolveExpect: function() {
    if (this.resolve && (this.invocations.length >= this.count)) {
      this.resolve(this.invocations.splice(0, this.count));
  ...
  expect: function(count) {
    if (this.invocations.length >= count) {
      var events = this.invocations.splice(0, count);
