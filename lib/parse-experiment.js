...
ParseExperiment.prototype.parse = function(data) {
  ...
  var pos = 0;
  var experiment = {inputs: [], tree: {}, flags: {}, options: {}};
  ...
  while (pos < lines.length && lines[pos].trim().length == 0)
    pos += 1;

  while (pos < lines.length) {
    var kv = lines[pos].split(':');
    var key = kv[0].trim();
    var value = kv.slice(1).join(':').trim();
    var keyParts = key.split('.');
    if (keyParts.length == 2) {
      if (experiment.options[keyParts[0]] == undefined)
        experiment.options[keyParts[0]] = {}
      experiment.options[keyParts[0]][keyParts[1]] = eval(value);
    } else
      experiment.flags[key] = value;
    pos += 1;
  }
