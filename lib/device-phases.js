...
var register = require('../core/phase-register');
...
register({name: 'fetch', input: types.string, output: types.JSON, arity: '1:1', async: true, parallel: false},
  function(url) {
    return telemetryTask('save.py', ['--browser=' + this.options.browser, '--', url]);
  },
  {browser: 'system'});

register({name: 'fetchWithInlineStyle', input: types.string, output: types.JSON, arity: '1:1', async: true, parallel: false},
  function(url) {
    return telemetryTask('save-no-style.py', ['--browser=' + this.options.browser, '--', url]);
  },
  {browser: 'system'});

register({name: "traceURL", input: types.string, output: types.JSON, arity: "1:1", async: true, parallel: false},
  function(url) {
    return telemetryTask('perf.py', ['--browser=' + this.options.browser, '--', url]);
  },
  {browser: 'system'});

register({name: "trace", input: types.string, output: types.JSON, arity: "1:1", async: true, parallel: false},
    function(html) {
      return hostedTelemetryTask(html, 'perf.py', ['--browser=' + this.options.browser, '--', 'http://localhost:8000']);
    },
    {browser: 'system'});

register({name: "traceLayout", input: types.string, output: types.JSON, arity: "1:1", async: true, parallel: false},
    function(html) {
