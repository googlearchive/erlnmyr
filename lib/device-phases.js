...
function BrowserWrapper(url, options) {
  this.task = spawn('python', ['telemetry/newBrowser.py', '--browser=' + options.browser]);
  this.url = url;
  this.data = undefined;
  ...
  this.task.on('close', function(code) { console.log('done'); });
}
...
module.exports.browser = phase({input: types.string, output: types.Browser(types.string), arity: "1:1"},
    function(url) {
      return new BrowserWrapper(url, this.options);
    },
    {browser: 'system'});
...
for (var i = 0; i < browserCommands.length; i++) {
  ...
  (function(cmd) {
    module.exports[cmd.name] = phase({input: types.Browser(cmd.input), output: types.Browser(cmd.output), arity: "1:1", async: true, parallel: false},
      function(browser) {
