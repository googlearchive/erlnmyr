...
if (options.chromium !== undefined) {
  ...
  process.env.PYTHONPATH += path.delimiter + chromium_path + '/third_party/catapult/telemetry';
...
function _telemetryTask(pyScript, pyArgs) {
  return new Promise(function(resolve, reject) {
    var result = "";
    pyScript = path.join(__dirname, '../erlnmyr_telemetry/' + pyScript);
...
function BrowserWrapper(url, options, resolve) {
  console.log(options);
  var cmd = ['erlnmyr_telemetry/newBrowser.py', '--browser=' + options.browser, '--chromium', chromium_path];
  if (options.perf) {
    cmd.push('--perf');
  }
